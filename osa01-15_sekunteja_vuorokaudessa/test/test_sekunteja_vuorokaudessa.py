...
class SekuntejaVuorokaudessaTest(unittest.TestCase):
    ...
    def test_yhden_vuodokauden_sekunnit(self):
        with patch('builtins.input', return_value = '1'):
            reload_module(self.module)
            result = get_stdout()
            self.assertTrue('86400' in result.split(), '1 vuorokaudessa tulisi olla 86400 sekuntia. Ohjelmasi palautti: ' + parse_result(result))

    def test_seitseman_vuorokauden_sekunnit(self):
        with patch('builtins.input', return_value = '7'):
            reload_module(self.module)
            result = get_stdout()
            self.assertTrue('604800' in result.split(), '7 vuorokaudessa tulisi olla 604800 sekuntia. Ohjelmasi palautti: ' + parse_result(result))
