    ...
    def test_eka_suurempi(self):
        ...
        for valuepair in values:
            with patch('builtins.input', side_effect = list(valuepair)):
                ...
                self.assertTrue(output[0].find(valuepair[0]) > -1, "Tulostuksesta {} ei löydy suurempaa lukua {} kun syöte on {}".format(output[0], valuepair[0], format_tuple(valuepair)))
    ...
    def test_toka_suurempi(self):
        ...
        for valuepair in values:
            with patch('builtins.input', side_effect = list(valuepair)):
                ...
                self.assertTrue(len(output) == 1, "Ohjelmasi tulostaa yhden rivin sijasta {} riviä syötteellä {}".format(len(output), format_tuple(valuepair)))
                self.assertTrue(output[0].find(valuepair[1]) > -1, "Tulostuksesta {} ei löydy suurempaa lukua {} kun syöte on {}".format(output[0], valuepair[1], format_tuple(valuepair)))
    ...
    def test_yhtasuuret(self):
        ...
        for valuepair in values:
            with patch('builtins.input', side_effect = list(valuepair)):
                ...
                self.assertTrue(len(output) == 1, "Ohjelmasi tulostaa yhden rivin sijasta {} riviä syötteellä {}".format(len(output), format_tuple(valuepair)))
                self.assertTrue(output[0].find("yhtä suuret") > -1, "Tulostuksesta {} ei löydy mainintaa 'yhtä suuret' kun syöte on {}".format(output[0], format_tuple(valuepair)))
