    ...
    def test_pitkat_sanat(self):
        ...
        for word in words:
            with patch('builtins.input', return_value = word):
                ...
                self.assertEqual(output[1], "Kiitos!", "Ohjelmasi ei tulosta lopuksi Kiitos! syötteellä " +word)  

    def test_yksittainen_merkki_ja_tyhja(self):
        words = "a X z".split(" ")
        words.append("")
        for word in words:
            with patch('builtins.input', return_value = word):
                ...
                self.assertFalse(len(output_all)==0, "Ohjelmasi ei tulosta mitään syötteellä "  +word)   
                self.assertTrue(len(output) == 1, "Ohjelmasi tulostaa yhden rivin sijasta riviä sijasta {} riviä: \n{}".format(len(output), word))
                self.assertEqual(output[0], "Kiitos!", "Ohjelmasi ei tulosta viestiä Kiitos! vaan " + output[0] + " syötteellä " +word)  
